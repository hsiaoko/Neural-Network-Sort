...
void ToBucket(KeysLogits *input, InitResultGpu *initResultGpu)
{
    ...
    for (int i = 0; i < input->sizeKeys; i++)
    {
        ...
        else
        {
            ...
            {
                initResultGpu->waitedList = (float *)realloc(initResultGpu->waitedList, sizeof(float) * (currentSize + 1));
            }
...
void Merge(InitResultGpu *initResult, float *finalResult)
{
    // cout << endl
    //      << "******************************initresult*************" << endl;
    // for (int i = 0; i < initResult->sizeSorted; i++)
    // {
    //     if (*(initResult->sortedList + i) != FLT_MAX)
    //         cout << *(initResult->sortedList + i) << "  ";
    // }
    int indexa = 0;
    int indexb = 0;
    int indexc = 0;
